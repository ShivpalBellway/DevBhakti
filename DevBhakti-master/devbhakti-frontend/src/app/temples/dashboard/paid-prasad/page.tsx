    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
        ...
        <div className="md:col-span-1 space-y-6">
          <Card className="border-[#7b4623]/20 shadow-md rounded-2xl overflow-hidden bg-white sticky top-24">
            <CardHeader className="bg-[#7b4623] text-white p-4">
              <div className="flex items-center gap-2 text-xs font-semibold tracking-wider text-amber-200 uppercase">
                <ShoppingBag className="w-4 h-4" /> Live Booking Preview
              </div>
              <CardTitle className="text-sm font-bold text-white mt-1">
                Devotee Booking View (Step 3)
              </CardTitle>
            </CardHeader>

            <CardContent className="p-4 space-y-4 text-xs">
              {hasPaidPrasad && numericPrice > 0 ? (
                <div className="p-4 rounded-xl border border-[#7b4623]/30 bg-[#7b4623]/5 space-y-3">
                  <div className="font-bold text-slate-800 flex items-center gap-1.5 text-xs">
                    <span>🙏</span> Would you like Prasad delivered to your home?
                  </div>

                  <div className="space-y-2 pt-1">
                    <label className="flex items-center gap-2 p-2 rounded-lg border border-[#7b4623]/40 bg-white font-medium text-slate-800 cursor-pointer shadow-xs">
                      <input type="radio" checked readOnly className="accent-[#7b4623]" />
                      <span>Yes (₹{numericPrice} per box)</span>
                    </label>
                    <label className="flex items-center gap-2 p-2 rounded-lg border border-slate-200 bg-white font-normal text-slate-500 cursor-pointer">
                      <input type="radio" disabled className="accent-[#7b4623]" />
                      <span>No</span>
                    </label>
                  </div>

                  {/* Quantity selector preview */}
                  <div className="flex items-center justify-between p-2.5 bg-white rounded-lg border border-[#7b4623]/20 mt-2">
                    <span className="font-semibold text-slate-700">Quantity:</span>
                    <div className="flex items-center border border-slate-300 rounded-md overflow-hidden bg-slate-50 font-bold">
                      <span className="px-2 py-0.5 text-slate-500">-</span>
                      <span className="px-3 py-0.5 bg-white text-slate-800">2</span>
                      <span className="px-2 py-0.5 text-slate-500">+</span>
                    </div>
                  </div>

                  {/* Price Calculation preview */}
                  <div className="pt-2 border-t border-[#7b4623]/20 flex justify-between items-center text-xs">
                    <span className="text-slate-600">Prasad Amount (2 × ₹{numericPrice}):</span>
                    <span className="font-bold text-[#7b4623] text-sm">₹{numericPrice * 2}</span>
                  </div>
                </div>
              ) : (
                <div className="p-6 rounded-xl border border-dashed border-slate-300 bg-slate-50 text-center space-y-2 text-slate-500">
                  <Package className="w-8 h-8 mx-auto text-slate-400 opacity-60" />
                  <p className="font-medium text-xs">Paid Prasad is currently Disabled.</p>
                  <p className="text-[11px] text-slate-400">Devotees will not see paid prasad options during pooja booking.</p>
                </div>
              )}

              {/* Summary Mock */}
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-1.5 text-[11px]">
                <div className="font-bold text-slate-700 border-b pb-1">Payment Summary Preview</div>
                <div className="flex justify-between text-slate-600">
                  <span>Pooja Package:</span>
                  <span>₹501</span>
                </div>
                {hasPaidPrasad && numericPrice > 0 && (
                  <div className="flex justify-between font-bold text-[#7b4623]">
                    <span>Prasad (2 × ₹{numericPrice}):</span>
                    <span>₹{numericPrice * 2}</span>
                  </div>
                )}
                <div className="flex justify-between font-extrabold text-slate-900 border-t pt-1 text-xs">
                  <span>Total Amount:</span>
                  <span>₹{501 + (hasPaidPrasad ? numericPrice * 2 : 0)}</span>
                </div>
              </div>
            </CardContent>
          </Card>
