                                    ...
                                    <div className="pt-6 border-t border-slate-100 space-y-4">
                                        ...
                                        <div className="p-4 bg-orange-50/50 rounded-2xl border border-orange-100/50 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                                            <div>
                                                ...
                                                <select
                                                    className="w-full h-10 px-3 rounded-lg border bg-white text-sm font-medium mt-1 focus:ring-1 focus:ring-primary focus:outline-none text-[#794A05]"
                                                    value={selectedBooking.prasadStatus || 'PREPARING'}
                                                    onChange={async (e) => {
                                                        const newStatus = e.target.value;
                                                        try {
                                                            const res = await updateBookingStatus(selectedBooking.id, { prasadStatus: newStatus });
                                                            if (res.success) {
                                            ...
                                            <div>
                                                <label className="text-[10px] font-bold text-slate-400 uppercase text-slate-400">Courier Name</label>
                                                <input
                                                    className="w-full h-10 px-3 rounded-lg border bg-white text-sm font-medium mt-1 text-slate-700"
                                                    placeholder="e.g. BlueDart"
                                                    value={selectedBooking.courierName || ""}
                                                    onChange={(e) => setSelectedBooking({ ...selectedBooking, courierName: e.target.value })}
                                                    onBlur={async () => {
                                                        await updateBookingStatus(selectedBooking.id, { courierName: selectedBooking.courierName });
                                                    }}
                                                />
                                            </div>
                                            <div>
                                                <label className="text-[10px] font-bold text-slate-400 uppercase text-slate-400">AWB/Tracking Code</label>
                                                <input
                                                    className="w-full h-10 px-3 rounded-lg border bg-white text-sm font-medium mt-1 text-slate-700"
                                                    ...
                                                    onBlur={async () => {
                                                        await updateBookingStatus(selectedBooking.id, { awbCode: selectedBooking.awbCode });
                                                    }}
                                                />
                                            </div>
                                            <div>
                                                <label className="text-[10px] font-bold text-slate-400 uppercase text-slate-400">Tracking Link</label>
                                                <input
                                                    className="w-full h-10 px-3 rounded-lg border bg-white text-sm font-medium mt-1 text-slate-700"
                                                    placeholder="https://..."
                                                    ...
                                                    onBlur={async () => {
                                                        await updateBookingStatus(selectedBooking.id, { trackingUrl: selectedBooking.trackingUrl });
                                                    }}
                                                />
                                            </div>
                                        </div>
