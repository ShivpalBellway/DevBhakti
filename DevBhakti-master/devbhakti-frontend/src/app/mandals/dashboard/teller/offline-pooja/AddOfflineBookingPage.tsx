...
export default function AddOfflineBookingPage({ onBack }: { onBack?: () => void }) {
  ...
  const [selectedMandal, setselectedMandal] = useState(searchParams.get("temple") || "");
  ...
  }, [selectedDate, selectedMandal, selectedPooja]);
    ...
    const loadData = async () => {
      try {
        ...
        if (savedUser) {
          ...
          setselectedMandal(user.ownerId);
          setFormData(prev => ({
            ...prev,
  ...
  useEffect(() => {
    ...
    if (!selectedMandal) {
      const platformCopy = allPoojas.find(p => p.masterPoojaId === masterId && p.mandalId === null && !p.isMaster);
      if (platformCopy) {
    ...
    if (String(currentPoojaData.mandalId) === String(selectedMandal)) return;

    // Look for this master pooja's copy in the currently selected temple
    const templeSpecificPooja = allPoojas.find(p => String(p.mandalId) === String(selectedMandal) && String(p.masterPoojaId) === String(masterId));
    ...
    if (templeSpecificPooja && templeSpecificPooja.id !== selectedPooja) {
      ...
      const platformCopy = allPoojas.find(p => String(p.masterPoojaId) === String(masterId) && (p.mandalId === null || p.mandalId === undefined));
  ...
  }, [selectedMandal, selectedPooja, allPoojas]);
  ...
  const platformPoojaOption = React.useMemo(() => {
    ...
    const platformCopy = allPoojas.find((p: any) => p.masterPoojaId === poojaFamilyId && p.mandalId === null && !p.isMaster);
  ...
  const sourceOptions = React.useMemo(() => {
    ...
    allMandals.forEach((temple: any) => {
      const templeSpecificPooja = allPoojas.find((p: any) => {
        if (p.mandalId !== temple.id) return false;
        if (poojaFamilyId) {
    ...
    return options;
  }, [allPoojas, allMandals, language, platformPoojaOption, poojaFamilyId, requestedPoojaParam, selectedPoojaData]);
  const selectedSourceKey = selectedMandal || (selectedPoojaData && (selectedPoojaData.isMaster || selectedPoojaData.mandalId === null) ? "platform" : "");

  // If selected pooja is a Master Pooja or Platform copy, show DevBhakti as the platform instead of temple dropdown
  const isMasterPoojaSelected = selectedPoojaData && (selectedPoojaData.isMaster || (selectedPoojaData.mandalId === null && !selectedPoojaData.isMaster));
  ...
  const handleSourceSelect = (option: {
    ...
    setselectedMandal(option.mandalId);
    if (option.poojaId) {
      setSelectedPooja(option.poojaId);
      router.push(`/booking?pooja=${option.poojaId}&temple=${option.mandalId}`);
    } else {
      setSelectedPooja("");
      router.push(`/booking?mandal=${option.mandalId}`);
    }
  };
    ...
    const fetchFee = async () => {
      ...
      try {
        const response = await fetch(`${API_URL}/bookings/calculate-commission`, {
          ...
          body: JSON.stringify({
            amount: basePrice,
            vendorType: selectedMandal ? 'TEMPLE' : 'GLOBAL',
            vendorId: selectedMandal || undefined,
            category: 'POOJA'
          })
        });
        const data = await response.json();
  ...
  const handleConfirmBooking = async () => {
    try {
      ...
      const bookingData = {
        ...
        mandalId: selectedMandal || undefined,
      ...
      const res = await createOfflineBookingMandal(bookingData);
  ...
  const handleDownloadReceipt = async () => {
    if (createdBooking) {
      const txId = createdBooking.transactionRef || createdBooking.id;
      if (txId) {
        try {
          ...
          if (res.ok) {
            ...
            a.href = url;
            const cleanReceiptNo = (createdBooking.id || txId.slice(-8)).replace(/[^a-zA-Z0-9_-]/g, "");
          ...
          {step === 3 && (
            <Card className="border-border/50">
              ...
              <CardContent className="space-y-4">
                ...
                {/* General address â€” hidden when prasad delivery address is filled */}
                {!isPrasadRequested && (
                ...
                {/* Structured Prasad Delivery Address â€” only shown when prasad is requested */}
                {isPrasadRequested && (
                  <div className="mt-4 p-4 rounded-2xl border border-orange-200 bg-orange-50/40 space-y-4">
                    <p className="text-sm font-bold text-[#794A05] flex items-center gap-2">
                      ðŸ“¦ {t("booking_client.prasad_delivery_title")}
                ...
                {selectedPoojaData?.hasPrasad && (
                  <div className="mt-6 p-4 border rounded-xl bg-orange-50/50 border-orange-100">
                    <Label className="text-sm font-bold text-slate-800 flex items-center gap-2">
                      ðŸ™ {t("booking_client.prasad_question")}
                    </Label>
                    <RadioGroup
...
{selectedMandal && (
  <div className="flex justify-between">
    ...
    <span className="font-medium text-right">
      {parseLocalizedValue(allMandals.find(t => t.id === selectedMandal)?.name, language) || ''}
    </span>
  </div>
)}
<div className="flex justify-between">
          ...
          {step === 5 && (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center space-y-6 py-8">
              ...
              <div className="flex gap-4 justify-center pt-8">
                <Button 
                    onClick={() => router.push('/mandals/dashboard/teller')} 
                    variant="outline" 
                    className="gap-2"
                  >
                    <ArrowLeft className="w-4 h-4" />
