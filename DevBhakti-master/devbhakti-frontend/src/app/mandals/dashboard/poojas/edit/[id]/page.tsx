...
export default function MandalEditPoojaPage() {
    ...
    const loadPooja = async () => {
        ...
        try {
            ...
            if (found) {
                const descriptions = found.description ? (typeof found.description === 'string' ? JSON.parse(found.description) : found.description) : [];
                const benefits = found.benefits ? (typeof found.benefits === 'string' ? JSON.parse(found.benefits) : found.benefits) : [];
                const bullets = found.bullets ? (typeof found.bullets === 'string' ? JSON.parse(found.bullets) : found.bullets) : [];
                const packages = found.packages ? (typeof found.packages === 'string' ? JSON.parse(found.packages) : found.packages) : [];
                const faqs = found.faqs ? (typeof found.faqs === 'string' ? JSON.parse(found.faqs) : found.faqs) : [];
                const processSteps = found.processSteps ? (typeof found.processSteps === 'string' ? JSON.parse(found.processSteps) : found.processSteps) : [];

                setFormData({
                    name_en: found.name?.en || found.name || "",
                    name_hi: found.name?.hi || "",
                    name_mr: found.name?.mr || "",
                    price: found.price || 0,
                    category: found.category?.en || found.category || "",
                    time: found.time || "",
                    about_en: found.about?.en || found.about || "",
                    about_hi: found.about?.hi || "",
                    about_mr: found.about?.mr || "",
                    description_en: descriptions[0] || [],
                    description_hi: descriptions[1] || [],
                    description_mr: descriptions[2] || [],
                    benefits_en: benefits[0] || [],
                    benefits_hi: benefits[1] || [],
                    benefits_mr: benefits[2] || [],
                    bullets_en: bullets[0] || [],
                    bullets_hi: bullets[1] || [],
                    bullets_mr: bullets[2] || [],
                    packages: packages,
                    processSteps: processSteps,
                    faqs: faqs,
                    hasPrasad: found.hasPrasad || false,
                    status: found.status ?? true
                });

                if (found.category) {
                    const catString = found.category?.en || found.category || "";
                    setSelectedCats(catString.split(",").map((s: string) => s.trim()).filter(Boolean));
                }

                if (found.image) {
                    setImagePreview(`${API_URL.replace('/api', '')}${found.image}`);
                }
            } else {
            ...
            console.error("Load Pooja Error:", error);
            toast({ title: "Error", description: "Failed to load pooja details", variant: "destructive" });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            submissionData.append('description', JSON.stringify([
                ...
                formData.description_mr
            ]));
            submissionData.append('benefits', JSON.stringify([
                formData.benefits_en,
                formData.benefits_hi,
                formData.benefits_mr
            ]));
            ...
            const res = await updateMandalPooja(poojaId, submissionData);

            if (res.success) {
                toast({ title: "Success", description: "Pooja updated successfully!" });
                router.push("/mandals/dashboard/poojas");
            } else {
                toast({ title: "Error", description: res.message || "Failed to update pooja", variant: "destructive" });
            }
        } catch (error: any) {
    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
                ...
                <p className="text-muted-foreground">Loading offering data...</p>
            </div>
        );
    }
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6 pb-20 px-2 sm:px-0">
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-2xl sm:text-3xl font-serif font-bold text-[#7b4623]">Edit Pooja</h1>
                    <p className="text-muted-foreground text-sm">Update the details of your mandal's seva ritual.</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm space-y-4">
                    ...
                    <div className="flex items-center gap-6 p-4 bg-orange-50/20 border border-orange-100 rounded-2xl">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            />
                            <Label htmlFor="status" className="font-bold text-xs text-slate-700 cursor-pointer">
                            ...
                            <TabsContent key={lang} value={lang} className="space-y-4 mt-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Textarea
                                        ...
                                        rows={4}
                                        value={formData[`description_${lang}`]?.join('\n') || ""}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Textarea
                                        ...
                                        rows={3}
                                        value={formData[`benefits_${lang}`]?.join('\n') || ""}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* Process Steps */}
                    <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm space-y-4">
                        <h2 className="text-lg font-bold text-slate-800 border-b pb-2">5. Steps involved</h2>
                        <div className="space-y-3">
                            <Input
                                placeholder="Step Title (e.g. Sankalp path)"
                                value={newStep.title}
                            ...
                            <Textarea
                                ...
                                rows={2}
                            />
                            <Button type="button" size="sm" onClick={addStep} className="bg-orange-50 text-[#7b4623] hover:bg-orange-100 w-full h-8 text-xs font-bold">
                                + Add Step
                              </Button>
                        </div>
                        ...
                        <div className="space-y-2 mt-4 max-h-[250px] overflow-y-auto pr-1">
                            {formData.processSteps.map((step, idx) => (
                                <div key={idx} className="p-3 bg-slate-50 rounded-xl relative border">
                                    <h4 className="font-bold text-slate-800 text-xs pr-6">{idx + 1}. {step.title}</h4>
                    ...
                    {/* FAQs */}
                    <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm space-y-4">
                        <h2 className="text-lg font-bold text-slate-800 border-b pb-2">6. Frequently Asked Questions</h2>
                        <div className="space-y-3">
                            <Input
                ...
                <div className="flex gap-4 items-center justify-end">
                    ...
                    <Button type="submit" disabled={isSubmitting} className="bg-[#7b4623] hover:bg-[#5d351a] text-white rounded-xl px-8 shadow-md">
                        {isSubmitting ? (
                            <>
                                <Loader2 className="w-4 h-4 mr-2 animate-spin" /> Saving Pooja...
                            </>
                        ) : (
                            "Save Changes"
                        )}
