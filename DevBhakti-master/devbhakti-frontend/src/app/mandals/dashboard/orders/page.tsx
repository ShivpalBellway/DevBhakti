...
import { fetchMandalOrders, updateMandalOrderStatus, fetchMandalProfile } from "@/api/mandalAdminController";
import { BASE_URL } from "@/config/apiConfig";
import { Checkbox } from "@/components/ui/checkbox";
import { parseLocalizedValue } from "@/utils/textUtils";

function MandalOrdersClient() {
    ...
    const loadMandalAndOrders = async () => {
        setIsLoading(true);
        try {
            // 1. Get Mandal Profile to get mandalId
            const profileRes = await fetchMandalProfile();
            if (profileRes.success && profileRes.data.id) {
                const id = profileRes.data.id;
                setMandalId(id);
                // 2. Load orders for this mandal
                setMandalData(profileRes.data);
                const ordersRes = await fetchMandalOrders(id);
                ...
                toast({
                    title: "Error",
                    description: "Failed to load mandal profile",
                    variant: "destructive",
                });
            }
        } catch (error) {
    ...
    const handleStatusUpdate = async (subOrderId: string, status: string) => {
        if (!mandalId) return;
        try {
            const response = await updateMandalOrderStatus(subOrderId, { status, mandalId });
            if (response.success) {
                toast({
                ...
                // Refresh data
                const ordersRes = await fetchMandalOrders(mandalId);
                if (ordersRes.success) {
                    setOrders(ordersRes.data);
    ...
    const handleBulkPrint = () => {
        ...
        router.push(`/mandals/dashboard/orders/print?ids=${ids}`);
    };

    const handleSinglePrint = (order: any) => {
        router.push(`/mandals/dashboard/orders/print?ids=${order.id}`);
    };
    ...
    const handleExportExcel = () => {
        ...
        XLSX.writeFile(wb, `Mandal_Orders_${mandalData?.name?.en || 'Export'}_${new Date().toISOString().split('T')[0]}.xlsx`);
    };
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <p className="text-slate-500 mt-1 font-medium">
                        Track and fulfill marketplace orders specifically for your mandal's offerings.
                    </p>
                </div>
                            ...
                            <TableRow>
                                <TableCell colSpan={7} className="h-48 text-center">
                                    <div className="flex flex-col items-center gap-2 text-slate-400">
                                        ...
                                        <p className="font-bold uppercase tracking-widest text-[10px]">No orders found for your mandal</p>
                                    </div>
                                </TableCell>
                            </TableRow>
                        ) : (
                                                            ...
                                                            <div className="flex items-center gap-4">
                                                                <div className="w-12 h-12 rounded-xl border border-slate-100 overflow-hidden bg-white shadow-sm flex-shrink-0">
                                                                    <img
                                                                        ...
                                                                        alt={parseLocalizedValue(item.product?.name)}
...
export default function MandalOrdersPage() {
    return (
        <Suspense fallback={
            <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
