...
export default function CreateProductPage() {
  ...
  const validateForm = () => {
    ...
    if (!formData.name_en.trim()) newErrors.name_en = "Product name in English is required";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Error", description: error?.response?.data?.message || error?.message || "Failed to create product", variant: "destructive" });
    } finally {
  ...
  const updateVariant = (id: string, field: keyof Variant, value: string | number) => {
    setVariants(variants.map(v => v.id === id ? { ...v, [field]: typeof value === "string" && (field === "price" || field === "stock") ? Number(value) : value } : v));
  };
  ...
  // Helper: get lang-specific field name
                  ...
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
                      <Label htmlFor={`name-${lang}`}>
                        {t("admin.products.product_name")} {lang === "en" && <span className="text-red-500">*</span>}
                      </Label>
                      <Input
                        id={`name-${lang}`}
                    ...
                    <div className="space-y-2">
                      <Label htmlFor={`description-${lang}`}>
                        {t("admin.products.description")} {lang === "en" && <span className="text-red-500">*</span>}
                      </Label>
                      <RichTextEditor
                    ...
                    <div className="space-y-2">
                      <Label htmlFor={`highlights-${lang}`}>{t("admin.products.highlights")}</Label>
                      <Textarea
                        id={`highlights-${lang}`}
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div className="flex items-center gap-4">
                          {productImagePreview ? (
                            <div className="relative">
                              <img src={productImagePreview} alt="Product preview" className="w-24 h-24 object-cover rounded-lg border" />
                              <Button type="button" variant="destructive" size="icon" className="absolute -top-2 -right-2 h-6 w-6" onClick={() => { setProductImage(null); setProductImagePreview(""); }}>
                                <X className="w-3 h-3" />
                              </Button>
                            </div>
                          ) : (
                      ...
                      <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100 space-y-4">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          <div className="space-y-2">
                            <Label>{t("admin.products.weight")} *</Label>
                            <Input type="number" step="0.01" value={formData.weight} onChange={(e) => setFormData({ ...formData, weight: e.target.value })} placeholder="0.5" />
                          </div>
                          <div className="space-y-2">
                            <Label>{t("admin.products.length")} *</Label>
                            <Input type="number" value={formData.length} onChange={(e) => setFormData({ ...formData, length: e.target.value })} placeholder="10" />
                          </div>
                          <div className="space-y-2">
                            <Label>{t("admin.products.width")} *</Label>
                            <Input type="number" value={formData.width} onChange={(e) => setFormData({ ...formData, width: e.target.value })} placeholder="10" />
                          </div>
                          <div className="space-y-2">
                            <Label>{t("admin.products.height")} *</Label>
                            <Input type="number" value={formData.height} onChange={(e) => setFormData({ ...formData, height: e.target.value })} placeholder="10" />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-2">
                            ...
                            <Input 
                              ...
                              onChange={(e) => updateVariant(variant.id, f("name", lang) as keyof Variant, e.target.value)} 
