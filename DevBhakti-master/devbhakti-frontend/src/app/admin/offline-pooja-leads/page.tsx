...
export default function AdminOfflineUsersPage() {
  ...
  const loadLeads = async () => {
    try {
      setLoading(true);
      const res = await fetchOfflinePoojaLeadsAdmin({ search });
      if (res.success) {
        setLeads(res.data || []);
        setStats({
      ...
      toast({ title: "Error", description: "Failed to load offline users", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePrintPoojaReceipt = (item: any) => {
    const html = generatePoojaReceiptHTML({
      ...
      poojaName: typeof item.poojaName === 'object' ? item.poojaName.en : (item.poojaName || item.title || 'Pooja'),
      templeName: typeof item.templeName === 'object' ? item.templeName.en : (item.templeName || 'DevBhakti Portal'),
    ...
    <div className="p-6 space-y-6 max-w-[1400px] mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="border-border/60 bg-gradient-to-br from-blue-50/50 to-indigo-50/30">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Offline Poojas</p>
              <h3 className="text-3xl font-bold text-slate-800 mt-1">{stats.totalBookings}</h3>
              <p className="text-xs text-blue-600 font-medium mt-1">Counter Bookings</p>
            </div>
        ...
        <Card className="border-border/60 bg-gradient-to-br from-rose-50/50 to-pink-50/30">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-rose-600 font-medium mt-1">Manual Donations</p>
            </div>
        ...
        <Card className="border-border/60 bg-gradient-to-br from-emerald-50/50 to-teal-50/30">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-emerald-600 font-medium mt-1">Total Offline Collection</p>
            </div>
      ...
      <Card className="border-border/60">
        <CardHeader className="border-b bg-slate-50/50 px-6 py-4">
          <CardTitle className="text-lg font-semibold flex items-center justify-between">
            <span>Offline Users Directory ({leads.length})</span>
        ...
        <CardContent className="p-0">
          {leads.length === 0 ? (
            <div className="text-center py-12 text-slate-500">
              ...
              <p className="text-sm mt-1">Offline donations or bookings will automatically generate user records here.</p>
              <Link href="/admin/pooja-bookings/add-offline" className="inline-block mt-4">
                <Button className="bg-[#794A05] text-white">Add First Offline Booking</Button>
              </Link>
            </div>
          ) : (
                  ...
                  {leads.map((lead, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right space-x-2 whitespace-nowrap">
                        ...
                        <Link href={`/admin/pooja-bookings/add-offline?phone=${encodeURIComponent(lead.phone)}`}>
              ...
              <div key={idx} className="p-4 rounded-xl border border-slate-200 bg-slate-50/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-slate-500">
                    Temple: {parseLocalizedValue(item.templeName, 'en') || 'DevBhakti Portal'} | Date: {item.bookingDate || new Date(item.createdAt).toLocaleDateString()} | Status: {item.status}
                  </p>
                </div>
