...
export default function AddOfflineBookingPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const poojasRaw = await fetchPublicPoojas({ lang: language });
        ...
        setAllTemples(Array.isArray(templesData) ? templesData : (templesData?.data || []));
        setAllPoojas(Array.isArray(poojasData) ? poojasData : (poojasData?.data || []));
  ...
  const handleConfirmBooking = async () => {
    try {
      ...
      const res = await createOfflineBookingAdmin(bookingData);
                ...
                <div className="bg-[#794A05]/5 p-6 rounded-xl border border-[#794A05]/10 space-y-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-right">{selectedPoojaData?.name}</span>
                    </div>
                    {selectedTemple && (
                      <div className="flex justify-between">
                        <span className="text-slate-600">Temple</span>
                        <span className="font-medium text-right">{allTemples.find(t => t.id === selectedTemple)?.name || ''}</span>
                      </div>
                    )}
                    <div className="flex justify-between">
                      <span className="text-slate-600">Package</span>
                      <span className="font-medium text-right">{selectedPackageData?.name}</span>
                    </div>
...
{step === 5 && (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center space-y-6 py-8">
              ...
              <div className="flex gap-4 justify-center pt-8">
                <Button 
                  onClick={() => router.push('/admin/pooja-bookings')} 
                  variant="outline" 
                  className="gap-2"
                >
                  <ArrowLeft className="w-4 h-4" />
