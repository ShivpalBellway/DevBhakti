    ...
    try {
                ...
                if (!existingLead) {
                    await prisma.lead.create({
                        data: {
                            ...
                            status: 'CONVERTED',
                            notes: `Offline Pooja Booking (${packageName})`,
                            metadata: { offlineBooking: true, bookingDisplayId: displayId }
                        }
                    });
                } else {
                    await prisma.lead.update({
                        ...
                        data: {
                            status: 'CONVERTED',
                            notes: existingLead.notes ? `${existingLead.notes} | Offline Pooja (${packageName})` : `Offline Pooja (${packageName})`
                        }
                    });
                }
            } catch (leadError) {
        ...
        try {
            await prisma.auditLog.create({
                data: {
                    action: 'CREATE_OFFLINE_POOJA_BOOKING',
                    module: 'POOJA_BOOKING',
                    entityId: booking.id,
                    performedBy: user?.id || user?.email || 'ADMIN',
                    userRole: user?.role || user?.ownerType || 'ADMIN',
                    details: {
...
export const lookupDevoteeByPhone = async (req: Request, res: Response) => {
    try {
        ...
        // Fallback: check previous offline bookings for this phone number
        const lastBooking = await prisma.poojaBooking.findFirst({
            where: {
                OR: [
                    { devoteePhone: normalizedPhone },
                    { devoteePhone: cleanedPhone },
                    { devoteePhone: String(phone) }
                ]
            },
        ...
        if (lastBooking) {
            return res.status(200).json({
                ...
                data: {
                    name: lastBooking.devoteeName,
                    phone: lastBooking.devoteePhone,
                    email: lastBooking.devoteeEmail,
                    gothra: lastBooking.gothra,
                    kuldevi: lastBooking.kuldevi,
                    kuldevta: lastBooking.kuldevta,
                    address: lastBooking.address,
                    dob: lastBooking.dob,
                    anniversary: lastBooking.anniversary,
                    nativePlace: lastBooking.nativePlace
                }
            });
        }
        ...
        console.error("Lookup devotee error:", error);
        return res.status(500).json({ success: false, message: error.message });
    }
};

export const getOfflinePoojaLeads = async (req: Request, res: Response) => {
    try {
        const { search, templeId, page = '1', limit = '50' } = req.query;
        ...
        if (templeId) {
            bookingWhere.templeId = String(templeId);
            donationWhere.templeId = String(templeId);
        }
        ...
        // Process Offline Pooja Bookings
        ...
        offlineDonations.forEach((donation) => {
            ...
            const item = {
                ...
                templeName: (donation as any).temple?.name,
        ...
        console.error("Get offline pooja leads error:", error);
        return res.status(500).json({ success: false, message: error.message });
    }
};
