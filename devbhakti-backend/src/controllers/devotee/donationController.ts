...
export const getDonationReceipt = async (req: Request, res: Response) => {
    try {
        ...
        const donation = await prisma.donation.findFirst({
            where: { id, userId, status: 'SUCCESS' },
            include: {
                temple: true
            }
        });
        ...
        // --- Colors ---
        const primaryColor = '#7c4624'; // Using the theme color from DonationClient
        ...
        doc.moveDown(0.5);
        doc.fillColor(textColor).font('Helvetica-Bold').fontSize(12).text(donation.temple.name);
        doc.font('Helvetica').fontSize(10).text(donation.temple.location);
        ...
        if (donation.is80GRequired) {
            doc.fillColor(textColor).fontSize(9).font('Helvetica-Oblique').text('* This donation is eligible for tax benefit under section 80G.', 50, doc.y);
            doc.moveDown(2);
        }
